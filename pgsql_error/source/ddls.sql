...
CREATE PROCEDURE `insert_salary`(
  ...
  IN `in_gross` FLOAT64,
  IN `in_deduction` FLOAT64,
  IN `in_net_pay` FLOAT64)
BEGIN
...
PARTITION BY RANGE_N (
  `order_date` BETWEEN DATE '2010-01-01' AND '2016-12-31'
  EACH INTERVAL '1' DAY
);
...
REPLACE VIEW `employee_view` AS
  SELECT `employee_no`, `first_name`, `birth_date`, `joined_date`, `department_no`
  FROM `employee`;
